...
class DuplicateDetector(nn.Module):
    def __init__(self, embedding_model, embedding_dimension, output_dimension, residual_layers_count, pooling_method):
        ...
        if self.residual > 0:
            self.residual_layers = nn.Sequential([Residual(in_features=embedding_dimension, out_features=output_dimension) for _ in range(self.residual)])
    def forward(self, input_id1, input_id2, attention_mask1, attention_mask2):
        embeddings1 = self.pooling_method(self.embedding_model(input_id1, attention_mask1))
        embeddings2 = self.pooling_method(self.embedding_model(input_id2, attention_mask2))
        if self.residual > 0:
